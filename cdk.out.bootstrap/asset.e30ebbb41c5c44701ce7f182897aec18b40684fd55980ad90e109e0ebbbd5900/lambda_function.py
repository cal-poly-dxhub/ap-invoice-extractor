...
def process_document(event):
    ...
    try:
        ...
        # Extract text (handle encrypted PDFs gracefully)
        raw_text, text_error = extract_text_from_pdf(file_content)
...
def extract_text_from_pdf(file_content):
    """Extract text from PDF using PyMuPDF with proper encrypted-document handling"""
    try:
        with fitz.open(stream=file_content, filetype="pdf") as doc:
            # Some signed PDFs report as encrypted but don't actually require a password.
            if doc.needs_pass:
                # Try empty password (common for owner-only protection)
                if doc.authenticate("") == 0:
                    return None, "PDF is encrypted and requires a password to open."
        ...
        return None, "PDF is encrypted and cannot be opened without a password."
    except Exception as e:
